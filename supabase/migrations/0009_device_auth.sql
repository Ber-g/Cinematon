-- Kioskoscope — durcissement de l'auth Kiosk (CIN-002).
--
...
-- lié à SA Kiosk par `booths.device_user_id`. Des policies device MINIMALES lui donnent
...
-- Compte device (auth user) lié à cette Kiosk. Une Kiosk = un device.
...
-- Écriture : séances de SA Kiosk + lectures de son org (les plays des séances qu'il crée).
...
-- MAJ : le device lit et met à jour les déploiements de SA Kiosk (applique/rollback).
...
-- Heartbeat : le device met à jour SA Kiosk (version + dernier contact). Row-level scopé
